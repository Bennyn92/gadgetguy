...
class WC_Predictive_Search_Functions
{
	...
	public static function remove_special_characters_in_mysql( $field_name, $search_keyword = '' ) {
		...
		if ( 'broad' != $predictive_search_mode ) {
			...
			$sql_after = $wpdb->prepare( $field_name . " LIKE %s ", '%'.$search_keyword.'%' );
		}
		...
		foreach ( $woocommerce_search_special_characters as $special_symbol ) {
			...
			if ( 'both' == $replace_special_character ) {
				if ( 'broad' != $predictive_search_mode ) {
					...
					$sql_after .= " OR ". $wpdb->prepare( self::replace_mysql_command( $field_name, $special_symbol, 'ignore' ) . " LIKE %s ", '%'.$search_keyword.'%' );
					$sql_after .= " OR ". $wpdb->prepare( self::replace_mysql_command( $field_name, $special_symbol, 'remove' ) . " LIKE %s ", '%'.$search_keyword.'%' );
				}
			} else {
				if ( 'broad' != $predictive_search_mode ) {
					...
					$sql_after .= " OR ". $wpdb->prepare( self::replace_mysql_command( $field_name, $special_symbol, $replace_special_character ) . " LIKE %s ", '%'.$search_keyword.'%' );
				}
			}
	...
	public static function change_add_to_cart_url( $url, $product ) {
		if ( in_array( $product->product_type, array( 'simple', 'subscription' ) ) ) {
			$url = $product->is_purchasable() && $product->is_in_stock() ? remove_query_arg( 'added-to-cart', add_query_arg( 'add-to-cart', $product->id, get_permalink( $product->id ) ) ) : get_permalink( $product->id );
		} elseif ( 'variation' == $product->product_type ) {
			global $product;
			$parent_post = $product->id;
			$product->id = $product->variation_id;
			$variation_data = array_map( 'urlencode', $product->variation_data );
			$url            = $product->is_purchasable() && $product->is_in_stock() ? remove_query_arg( 'added-to-cart', add_query_arg( array_merge( array( 'variation_id' => $product->variation_id, 'add-to-cart' => $parent_post ), $variation_data ), get_permalink( $parent_post ) ) ) : get_permalink( $parent_post );
		}
